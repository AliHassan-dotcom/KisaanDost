#!/usr/bin/env python3
"""Create processed/Punjab_Monthly_Risk_Score.csv and reports/risk_trends.png.

Reads processed/Punjab_Monthly_Features.csv and computes a transparent,
rule-based Punjab-wide crop-stress risk score (0-1). This is NOT a
...
FEATURES_FILE = BASE_DIR / "processed" / "Punjab_Monthly_Features.csv"
OUT_FILE = BASE_DIR / "processed" / "Punjab_Monthly_Risk_Score.csv"
CHART_FILE = BASE_DIR / "reports" / "risk_trends.png"
...
SUSPICIOUS_ACTION = ("verify rainfall source before using this month "
                     "for drought conclusions")
...
def main() -> int:
    ...
    for r in rows:
        ...
        # --- component severities and sub-reasons ---
        subs = {}
    ...
    for o in out_rows:
        if o["risk_level"] != "Low" or o["data_quality_flag"] != "ok":
...
def make_chart(rows: list) -> bool:
    ...
    ax.set_title("Punjab-wide crop-stress risk (indicative), monthly 2024-01 to 2026-08\n"
    ...
    fig.text(0.99, 0.005, "2026 is partial (Jan-Aug); baselines from 2024-2025 monthly data",
